(function(){
  ...
  function renderChatView(){
    ...
    if(state.chatMessages.length === 0){
      html += '<div class="chat-welcome">' +
        ...
        '<div class="chat-welcome-title">Hi, SG</div>' +
      ...
      state.chatMessages.forEach(function(m){
        if(m.role === "user"){
          ...
          html += '<div class="bot-name-tag">Hi,SG</div>' +
  ...
  function renderJournalView(){
    var html = '<div class="banner">📝 <b>업무일지</b>는 Hi,SG에 물어본 내용이 자동으로 기록되는 공간이에요. 이동 중 음성으로 물어본 내용도 그대로 남으니, 복귀 후 메모만 덧붙여 정리하면 됩니다. <b>기록은 이 기기에만 저장</b>되며 외부로 전송되지 않습니다.</div>';
    if(!state.journal.length){
      return html + '<div class="empty-state"><div class="big">📝</div>아직 기록이 없어요.<br/>Hi,SG 탭에서 질문하면 자동으로 쌓입니다.</div>';
    }
  ...
  var NAV_ITEMS = [
    ...
    {id:"chat", icon:"💬", label:"Hi, SG", desc:"음성/대화 상담"},
